...
def predict():
    ...
    if file:
        ...
        try:
            ...
            # clamp to 0-100 to avoid invalid CSS widths
            confidence_pct = min(max(confidence_pct, 0.0), 100.0)
            result = {
                'prediction': CLASS_NAMES[predicted_class],
                # numeric percentage for styling (e.g. width) and a formatted text for display
                'confidence': confidence_pct,
                'confidence_text': f'{confidence_pct:.2f}%',
                'image_file': filename
            }
